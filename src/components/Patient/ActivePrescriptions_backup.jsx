...
const ActivePrescriptions = ({
  ...
  const handleChatInit = async (chatData) => {
    console.log("handleChatInit called with:", chatData);
    ...
    try {
      // Initialize chat thread if threadId is not provided
      let threadId = chatData?.threadId;
      if (!threadId) {
        console.log("No threadId provided, initializing chat...");
        const res = await axiosInstance.post("/chat/init", {
          prescriptionId: chatData?.prescriptionId,
          // Note: pharmacyId should be available in the prescription data
          pharmacyId:
            prescription?.pharmacyId || prescription?.selectedPharmacy?._id,
        });
        threadId = res.data.data.threadId;
        console.log("Chat initialized with threadId:", threadId);
      }
      ...
      console.log("Chat modal state set to:", {
        isOpen: true,
        threadId: threadId,
        pharmacyName: chatData?.pharmacyName,
        prescriptionId: chatData?.prescriptionId,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
      <div className="p-6 border-b border-gray-200 flex items-center justify-between">
        ...
        <button className="text-blue-600  text-sm font-medium flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-blue-50 transition-all duration-200">
